...
# map(factorial, range(1, 11)) : 함수 , 반복 가능한 수 두가지 인자를 받음
# reduce(add, range(1, 11)) : 누적 연산 가능
# partial(mul, 5) : 인수 고정 가능
# __call__ : callable 하게 하는 매직 메소드
# (*args, **kwargs)
# lambda
...
print('EX2-2 -', var_func(10)) # 실행시
print('EX2-3 -', map(var_func, range(1,11)))
print('EX2-4 -', list(map(var_func, range(1,6))))


# 함수를 인수로 전달 & 함수로 결과 반환 -> 고위 함수(Higher-order function)
# map, filter, reduce 등
